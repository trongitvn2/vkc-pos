...
'    End If
    SQL = "SELECT Instock_MasterB.Doc_Number, Instock_MasterB.DateTime, Instock_MasterB.Customer_ID," & _
        ...
        If Stock_ID = "02" Then
            With rsPLU
                ...
                If InStr(1, txtPluCode.Text, "*", vbTextCompare) > 0 Then
                    .Open "SELECT  PluCode, PluName,Cost, Unit FROM SetMPLU WHERE PluCode LIKE '%" & Left(txtPluCode.Text, Len(Trim(txtPluCode.Text)) - 1) & "' OR PluName LIKE '" & _
                    Left(Trim(txtPluCode.Text), Len(Trim(txtPluCode.Text)) - 1) & "%'  ORDER BY PluCode asc"
                Else
                    .Open "SELECT  PluCode, PluName, Cost, Unit FROM SetMPLU WHERE PluCode LIKE '" & Trim(txtPluCode.Text) & "%' OR  PluName LIKE '" & Trim(txtPluCode.Text) & "%' AND lTRIM(PluName)<> '' ORDER BY PluCode ASC"
                End If
            End With
        Else
            ...
            With rsPLU
                If .State = adStateOpen Then .Close
                If InStr(1, txtPluCode.Text, "*", vbTextCompare) > 0 Then
                    .Open "SELECT  ItemNum as PluCode, ItemName as PluName,Std_Price1 as Cost, Unit FROM Inventory WHERE ItemNum, LIKE '%" & Left(txtPluCode.Text, Len(Trim(txtPluCode.Text)) - 1) & "' OR ItemName LIKE '" & _
                    Left(Trim(txtPluCode.Text), Len(Trim(txtPluCode.Text)) - 1) & "%'  ORDER BY ItemNum asc"
                Else
                    .Open "SELECT  ItemNum as Plucode, ItemName as PluName,Std_Price1 as Cost, Unit FROM Inventory WHERE ItemNum LIKE '%" & Trim(txtPluCode.Text) & "' OR ItemName LIKE '%" & Trim(txtPluCode.Text) & "' AND LTRIM(ItemName)<> '' ORDER BY ItemNum ASC"
                End If
            End With
        End If
        With griPLU
