VERSION 5.00
Begin {BD4B4E61-F7B8-11D0-964D-00A0C9273C2A} crSaleReport 
   ClientHeight    =   9510
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   15450
   OleObjectBlob   =   "crSaleReport.dsx":0000
End
Attribute VB_Name = "crSaleReport"
            ...
            With rsInventory
                ...
                If Not .EOF Then
                    If ArrayFlag(.Fields("F1"), 3) = 1 Then
                        txtQty.DecimalPlaces = DecimalQtyNumber
                    Else
                        txtQty.DecimalPlaces = 0
                    End If

                    If Left(txtPlucode.Value, 3) <> "KAR" Then
...
Private Sub Section16_Format(ByVal pFormattingInfo As Object)
    ...
    Dim des() As String
    des = LoadLanguage(LngFile, "#02:003:")
    Select Case Trim(txtPaymentMethod.Value)
        Case "C":
            txtMethod.SetText des(3)
        Case "CT":
            txtMethod.SetText des(4)
        Case "GC":
            txtMethod.SetText des(5)
        Case "CC":
            txtMethod.SetText des(7)
        Case "OA":
            txtMethod.SetText des(6)
        Case "ROA":
            txtMethod.SetText des(8)
    End Select
End Sub
...
On Error GoTo Handle
    With rsLocation
        If .RecordCount > 0 Then
            .Find "Location_ID='" & Field5.Value & "'", , adSearchForward, adBookmarkFirst
            If Not .EOF Then
                lblName.SetText .Fields("Section_ID")
            Else
                lblName.SetText ""
            End If
        End If
