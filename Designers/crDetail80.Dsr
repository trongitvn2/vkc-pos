VERSION 5.00
Begin {BD4B4E61-F7B8-11D0-964D-00A0C9273C2A} crDetail80 
   ClientHeight    =   9510
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   10110
   OleObjectBlob   =   "crDetail80.dsx":0000
End
Attribute VB_Name = "crDetail80"
...
With rscash
    ...
    If Not .EOF Then
        lblCashName.SetText .Fields("UserName")
    End If
End With
Exit Sub
Handle:
...
On Error GoTo Handle
    ...
    With rsGroup
        .Find "Dept_ID='" & txtGroup.Value & "'", , adSearchForward, adBookmarkFirst
        If Not .EOF Then
            lblGroupName.SetText .Fields("Description")
        End If
    End With
Exit Sub
Handle:
MsgBox Err.Number & Err.Description & Me.name & " Lay ten nhom"
End Sub
...
On Error GoTo Handle
    ...
    If rscompany.RecordCount > 0 Then
        ...
        lblInfor4.SetText rscompany.Fields("Company_info_4")
        lblInfor5.SetText rscompany.Fields("Company_info_5")
