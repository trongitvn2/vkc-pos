VERSION 5.00
Begin {BD4B4E61-F7B8-11D0-964D-00A0C9273C2A} crSaleBillA5 
   ClientHeight    =   9510
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   16425
   OleObjectBlob   =   "crSaleBillA5.dsx":0000
End
Attribute VB_Name = "crSaleBillA5"
...
On Error GoTo Handle
    If Trim(txtCustomer.Value) = "101" Or txtCustomer.Value = "" Then
    ...
    With rscust
        .Find "CustNum='" & Trim(txtCustomer.Value) & "'", , adSearchForward, adBookmarkFirst
        If Not .EOF Then
...
With rsserver
    .Find "Location_ID='" & Trim(txtserver.Value) & "'", , adSearchForward, adBookmarkFirst
    If Not .EOF Then
...
On Error GoTo Handle
    lblDatetime.SetText gfCONVERT_STRING_TO_DATE(txtDateIn.Value)
